...
`r shiny::tags$h3(basename(filelist[[i]]), style="text-align:center;color:#ffffff")`
...
protein_heatmap_data <- 
  ...
  dplyr::rename("Fraction" = fraction)
...
proteoform_heatmap_data <- 
  ...
  dplyr::mutate(mass_bin =  (mass_bin * bin_size) - bin_size) %>% 
  dplyr::rename("Proteoform Count" = n) %>%
  dplyr::ungroup() %>% 
  dplyr::mutate(fraction = forcats::as_factor(fraction)) %>% 
  dplyr::mutate(`Mass Bin (kDa)` = mass_bin/1000) %>% 
  dplyr::rename("Fraction" = fraction)
